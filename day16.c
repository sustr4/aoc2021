#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <malloc.h>

char* getBinInput() {
	char *x0input, *input;
	int i;
	asprintf(&x0input,"");
	input=calloc(4*strlen(x0input),sizeof(char));

	for(i=0;i<strlen(x0input);i++) {
		if(x0input[i]>=(char)'8') input[i*4]=1;
		if(x0input[i]=='4' || x0input[i]=='5'  || x0input[i]=='6' || x0input[i]=='7' || x0input[i]=='C' || x0input[i]=='D' || x0input[i]=='E' || x0input[i]=='F') input[i*4+1]=1;
		if(x0input[i]=='2' || x0input[i]=='3'  || x0input[i]=='6' || x0input[i]=='7' || x0input[i]=='B' || x0input[i]=='B' || x0input[i]=='E' || x0input[i]=='F') input[i*4+2]=1;
		if(x0input[i]=='1' || x0input[i]=='3'  || x0input[i]=='5' || x0input[i]=='7' || x0input[i]=='A' || x0input[i]=='B' || x0input[i]=='D' || x0input[i]=='F') input[i*4+3]=1; }
	return input;
}

int main () {
	char* input;
	int i;

	input=getBinInput();

//	for(i=0;i<1332*4;i++) printf("%d",input[i]);



	return 0;
}
